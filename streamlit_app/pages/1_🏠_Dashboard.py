...
# Показываем роль пользователя для понимания прав доступа
user_role = user.get("role") if user else "guest"
role_display = {
    "admin": "👑 Администратор (видит все хозяйства)",
    "farmer": "🚜 Фермер",
    "viewer": "👁️ Наблюдатель"
}.get(user_role, "Гость")

st.caption(f"Добро пожаловать, **{get_user_display_name()}** | {role_display}")
